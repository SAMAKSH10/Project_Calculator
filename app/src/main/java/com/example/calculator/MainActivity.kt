package com.example.calculator

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.text.method.ScrollingMovementMethod
import android.widget.Button
import android.widget.TextView
import javax.script.ScriptEngine
import javax.script.ScriptEngineFactory
import javax.script.ScriptEngineManager

class MainActivity : AppCompatActivity() {
    private lateinit var expression:TextView
    private lateinit var result:TextView
    private lateinit var multiply:Button
    private lateinit var percentage:Button
    private lateinit var divide:Button
    private lateinit var add:Button
    private lateinit var subtract:Button
    private lateinit var dot:Button
    private lateinit var clear:Button
    private lateinit var backspace:Button
    private lateinit var one:   Button
    private lateinit var two:   Button
    private lateinit var three: Button
    private lateinit var four:  Button
    private lateinit var five:  Button
    private lateinit var six:   Button
    private lateinit var seven: Button
    private lateinit var eight: Button
    private lateinit var nine:  Button
    private lateinit var zero:  Button
    private lateinit var doublezero:  Button
    private lateinit var equal:  Button

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)
        expression=findViewById(R.id.expression)
        result=findViewById(R.id.result)
        multiply=findViewById(R.id.multiplication)
        percentage=findViewById(R.id.percentage)
        divide=findViewById(R.id.division)
        add=findViewById(R.id.addition)
        subtract=findViewById(R.id.subtraction)
        zero=findViewById(R.id.zero)
        one=findViewById(R.id.one)
        two=findViewById(R.id.two)
        equal=findViewById(R.id.equal)
        three=findViewById(R.id.three)
        four=findViewById(R.id.four)
        five=findViewById(R.id.five)
        six=findViewById(R.id.six)
        seven=findViewById(R.id.seven)
        eight=findViewById(R.id.eight)
        nine=findViewById(R.id.nine)
        dot=findViewById(R.id.dot)
        doublezero=findViewById(R.id.doublezero)
        clear=findViewById(R.id.clear)
        backspace=findViewById(R.id.backspace)

        expression.movementMethod=ScrollingMovementMethod()
        expression.isActivated=true
        expression.isPressed=true

        var str:String

        clear.setOnClickListener{
            expressionText("0")
            expression.textSize=60f
            result.textSize=30f
            resultText()
        }

        backspace.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(!expression.text.toString().isEmpty()){
                val lastIndex = expression.text.toString().lastIndex
                if(expression.text.toString().lastIndex==0){
                    str=expression.text.toString().replace(expression.text.toString(),"0")
                    expressionText(str)
                }
                    else {
                    str = expression.text.toString().substring(0, lastIndex)
                    expressionText(str)
                }
                resultText();
            }
            else{
                str=expression.text.toString()
                expressionText(str)
                resultText()
            }
        }

        percentage.setOnClickListener {
        if(expression.text.toString().endsWith("%") || expression.text.toString().endsWith("/") || expression.text.toString().endsWith("+") || expression.text.toString().endsWith("-") || expression.text.toString().endsWith("*") || expression.text.toString().endsWith(".")){
            str=expression.text.toString();
            expressionText(str);
                                }
        else{
            expression.textSize=60f
            result.textSize=30f
            str=expression.text.toString()+"%"
            expressionText(str);
            }
        }

        divide.setOnClickListener {
            if(expression.text.toString().endsWith("%") || expression.text.toString().endsWith("/") || expression.text.toString().endsWith("+") || expression.text.toString().endsWith("-") || expression.text.toString().endsWith("*") || expression.text.toString().endsWith(".")){
                str=expression.text.toString();
                expressionText(str);
            }
            else{
                expression.textSize=60f
                result.textSize=30f
                str=expression.text.toString()+"/"
                expressionText(str);
            }
        }

        multiply.setOnClickListener {
            if(expression.text.toString().endsWith("%") || expression.text.toString().endsWith("/") || expression.text.toString().endsWith("+") || expression.text.toString().endsWith("-") || expression.text.toString().endsWith("*") || expression.text.toString().endsWith(".")){
                str=expression.text.toString();
                expressionText(str);
            }
            else{
                expression.textSize=60f
                result.textSize=30f
                str=expression.text.toString()+"*"
                expressionText(str);
            }
        }

        add.setOnClickListener {
            if(expression.text.toString().endsWith("%") || expression.text.toString().endsWith("/") || expression.text.toString().endsWith("+") || expression.text.toString().endsWith("-") || expression.text.toString().endsWith("*") || expression.text.toString().endsWith(".")){
                str=expression.text.toString();
                expressionText(str);
            }
            else{
                expression.textSize=60f
                result.textSize=30f
                str=expression.text.toString()+"+"
                expressionText(str);
            }
        }

        subtract.setOnClickListener {
            if(expression.text.toString().endsWith("%") || expression.text.toString().endsWith("/") || expression.text.toString().endsWith("+") || expression.text.toString().endsWith("-") || expression.text.toString().endsWith("*") || expression.text.toString().endsWith(".")){
                str=expression.text.toString();
                expressionText(str);
            }
            else{
                expression.textSize=60f
                result.textSize=30f
                str=expression.text.toString()+"-"
                expressionText(str);
            }
        }

        dot.setOnClickListener {
            if(expression.text.toString().endsWith("%") || expression.text.toString().endsWith("/") || expression.text.toString().endsWith("+") || expression.text.toString().endsWith("-") || expression.text.toString().endsWith("*") || expression.text.toString().endsWith(".")){
                str=expression.text.toString();
                expressionText(str);
            }
            else{
                expression.textSize=60f
                result.textSize=30f
                str=expression.text.toString()+"."
                expressionText(str);
            }
        }

        equal.setOnClickListener {
            expression.textSize=30f
            result.textSize=60f
            expression.text=result.text.toString().substring(1)

        }
        zero.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"0"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "0"
                expressionText(str)
                resultText()
            }
        }
        doublezero.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"00"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "00"
                expressionText(str)
                resultText()
            }
        }
        one.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"1"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "1"
                expressionText(str)
                resultText()
            }
        }
        two.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text.toString().substring(1)){
                str=expression.text.toString().replace(expression.text.toString(),"")+"2"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "2"
                expressionText(str)
                resultText()
            }
        }
        three.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"3"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "3"
                expressionText(str)
                resultText()
            }
        }
        four.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"4"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "4"
                expressionText(str)
                resultText()
            }
        }
        five.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"5"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "5"
                expressionText(str)
                resultText()
            }
        }
        six.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"6"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "6"
                expressionText(str)
                resultText()
            }
        }
        seven.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"7"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "7"
                expressionText(str)
                resultText()
            }
        }
        eight.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"8"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "8"
                expressionText(str)
                resultText()
            }
        }
        nine.setOnClickListener {
            expression.textSize=60f
            result.textSize=30f
            if(expression.text.toString().startsWith("0")||expression.text==result.text){
                str=expression.text.toString().replace(expression.text.toString(),"")+"9"
                expressionText(str)
                resultText()
            }
            else{
                str=expression.text.toString() + "9"
                expressionText(str)
                resultText()
            }
        }
    }
    private fun expressionText(str:String){
        expression.text=str;
    }
    private fun resultText(){
    val exp=expression.text.toString()
        val engine: ScriptEngine = ScriptEngineManager().getEngineByName("rhino")
        try{
            val res=engine.eval(exp)
            if(res.toString().endsWith(".0")){
                result.text="="+res.toString().replace(".0","")
            }
            else{
                result.text="=$res"
            }
        }
        catch (e:Exception){
            expression.text=expression.text.toString()
            result.text=result.text.toString()
        }
    }
}